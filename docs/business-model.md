...
- `:reason :no-automation-path`, or `:location :remote`, or `:duration
  :one-off` → `cloud-itonami-isic-8299` (independent contracted operator,
  task-based, no employer-of-record)
- `:duration :permanent` → `cloud-itonami-isic-7810` (placement-fee
  agency; wins over a merely-recurring signal)
- `:duration :recurring` + `:location :on-site` → `cloud-itonami-isic-7820`
  (temp-staffing/dispatch, employer-of-record)
- anything else/ambiguous → conservative default, also
  `cloud-itonami-isic-8299` (the only target with no employer-of-record
  liability), with the fallback named explicitly in the draft's
  `:routing-reason` so it's never a silent guess

`cloud-itonami-isic-6399` (public job board) is deliberately NOT one of
`route-gap`'s branches — it is a separate, optional widen-reach step
(`kotoba.occupation/widen-reach-draft`) that a human may invoke explicitly
...
cloud-itonami actor's README already states. A task under
`:human-required` stays `:pending-referral` in this actor's own execution
state until a human actually carries the draft in and the target actor's
own governor takes over — it is never fabricated as complete.
